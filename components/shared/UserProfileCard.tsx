...
const UserProfileCard: React.FC<UserProfileCardProps> = ({ settings, className = '', layout = 'vertical', children }) => {
  ...
  return (
    <div
      className={`flex ${isHorizontal ? 'flex-row text-left gap-4 items-center' : 'flex-col items-center text-center'} p-3.5 rounded-2xl transition-all duration-300 ease-out relative overflow-hidden ${className}`}
      style={{
    ...
    >
      {/* 头像区域 */}
      <div
        className={`${isHorizontal ? 'w-20 h-20' : 'w-11 h-11'} rounded-full flex items-center justify-center transition-all duration-300 shrink-0 overflow-hidden`}
        style={{
      ...
      {/* 内容区域 */}
      <div className={`flex-1 min-w-0 ${isHorizontal ? 'flex flex-row items-center justify-between gap-2' : ''}`}>
        {/* 姓名与职位 */}
        <div className="flex flex-col justify-center min-w-0">
          <div className="text-[13px] font-bold text-white leading-tight">余炜铭</div>
          <div className="text-[10px] text-white/50 leading-tight mt-1">Rigger & TD</div>
        </div>
